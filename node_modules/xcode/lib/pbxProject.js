...
pbxProject.prototype.addFramework = function(fpath, opt) {
    ...
    if (customFramework) {
        ...
        if (embed) {
          // We actually need a second Framework entry to embed
          opt.sign = true;
          var fileEmbed = new pbxFile(fpath, opt);

          fileEmbed.uuid = this.generateUuid();
          fileEmbed.fileRef = file.fileRef;
          fileEmbed.target = file.target;
          fileEmbed.lastKnownFileType = "wrapper.framework";
          fileEmbed.explicitFileType = "wrapper.framework";
          fileEmbed.fileEncoding = "9";
          fileEmbed.group = "Embed Frameworks";

          this.addToPbxBuildFileSection(fileEmbed);        // PBXBuildFile
          this.addToPbxEmbedFrameworksBuildPhase(fileEmbed); // PBXCopyFilesBuildPhase
        }
    }
